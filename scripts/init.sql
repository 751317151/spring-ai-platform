...
INSERT INTO ai_bot_permissions (id, bot_type, allowed_roles, allowed_departments, daily_token_limit) VALUES
    (uuid_generate_v4()::text, 'rd',           'ROLE_RD,ROLE_ADMIN',      '研发中心,系统管理', 200000),
    (uuid_generate_v4()::text, 'sales',        'ROLE_SALES,ROLE_ADMIN',   '销售部,系统管理',   150000),
    (uuid_generate_v4()::text, 'hr',           'ROLE_HR,ROLE_ADMIN',      '人力资源部,系统管理',100000),
    (uuid_generate_v4()::text, 'finance',      'ROLE_FINANCE,ROLE_ADMIN', '财务部,系统管理',   100000),
    (uuid_generate_v4()::text, 'supply-chain', 'ROLE_USER,ROLE_ADMIN',    NULL,                100000),
    (uuid_generate_v4()::text, 'qc',           'ROLE_USER,ROLE_ADMIN',    NULL,                100000),
    (uuid_generate_v4()::text, 'multi',        'ROLE_ADMIN',              '系统管理',          500000)
ON CONFLICT DO NOTHING;

-- Default knowledge bases
INSERT INTO knowledge_bases (id, name, description, department, created_by) VALUES
    ('kb-001', '企业通用知识库', '公司政策、HR规定、行政制度等通用知识', '全公司',  'system'),
    ('kb-002', '研发技术知识库', '技术规范、API文档、架构设计、代码规范', '研发中心', 'system'),
    ('kb-003', '销售产品知识库', '产品手册、报价策略、客户案例、竞品分析', '销售部',  'system')
ON CONFLICT DO NOTHING;
