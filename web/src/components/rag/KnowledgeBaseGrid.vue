  ...
  <div>
    ...
    <div class="kb-toolbar">
      <input
        ...
        class="form-input kb-search"
        placeholder="按名称、描述、部门或负责人搜索知识库"
      >
    ...
    <SkeletonBlock
      v-if="ragStore.loadingKnowledgeBases"
      variant="grid"
      :count="6"
      :height="248"
      :min-width="220"
    ...
    <div v-else class="kb-grid">
      <button
        ...
        @click="handleKnowledgeBaseClick(kb.id)"
      >
        ...
        <div class="kb-tags">
          <span class="kb-tag kb-tag-scope" :class="scopeTone(kb.visibilityScope)">
            {{ visibilityScopeLabel(kb.visibilityScope) }}
          </span>
        ...
        <div class="kb-detail-grid">
          <div class="kb-detail-item">
            <div class="kb-detail-label">分块策略</div>
            <div class="kb-detail-value">
              {{ kb.chunkStrategy || (kb.chunkSize ? `${kb.chunkSize} / overlap ${kb.chunkOverlap ?? 0}` : '默认策略') }}
            </div>
          </div>
          <div class="kb-detail-item">
            <div class="kb-detail-label">创建时间</div>
            <div class="kb-detail-value">{{ formatDisplayTime(kb.createdAt) }}</div>
          </div>
        </div>
      </button>
    </div>
...
const hasFilters = computed(() =>
  Boolean(keyword.value || scopeFilter.value !== 'ALL' || sortMode.value !== 'documents')
)
const scopeFilterLabel = computed(() => {
  if (scopeFilter.value === 'PUBLIC') return '范围: 公共可见'
  if (scopeFilter.value === 'PRIVATE') return '范围: 仅创建人可见'
  if (scopeFilter.value === 'DEPARTMENT') return '范围: 部门内可见'
  return '范围: 全部'
})
const filteredKnowledgeBases = computed(() => {
  ...
  const filtered = ragStore.knowledgeBases.filter((kb) => {
    ...
    if (scopeFilter.value !== 'ALL' && normalizedScope !== scopeFilter.value) {
      return false
    }
    if (!normalizedKeyword) {
      return true
    }
    return [kb.name, kb.description, kb.department, kb.id, kb.createdBy]
      .filter(Boolean)
...
const currentKnowledgeBase = computed(() =>
  ragStore.knowledgeBases.find((kb) => kb.id === ragStore.currentKb) || null
)
...
const activeKnowledgeBaseLabel = computed(() =>
  ragStore.currentKbName ? `当前知识库: ${ragStore.currentKbName}` : '当前未选中知识库'
)
...
function openEditModal() {
  if (currentKnowledgeBase.value) {
    editingKnowledgeBaseId.value = currentKnowledgeBase.value.id
    modalVisible.value = true
  }
}
...
function handleSaved() {
  const isEditing = Boolean(editingKnowledgeBaseId.value)
  closeModal()
  showToast(isEditing ? '知识库已更新' : '知识库已创建')
}

function handleKnowledgeBaseClick(kbId: string) {
  const normalizedKbId = String(kbId)
  ragStore.selectKb(normalizedKbId)
  if (props.selectionMode === 'route') {
    void router.push({ name: 'rag-detail', params: { kbId: normalizedKbId } })
  }
}
...
function visibilityScopeLabel(scope?: string) {
  const normalized = (scope || 'DEPARTMENT').toUpperCase()
  if (normalized === 'PUBLIC') return '公共可见'
  if (normalized === 'PRIVATE') return '仅创建人可见'
  return '部门内可见'
}

function scopeTone(scope?: string) {
  const normalized = (scope || 'DEPARTMENT').toUpperCase()
  if (normalized === 'PUBLIC') return 'scope-public'
  if (normalized === 'PRIVATE') return 'scope-private'
...
.kb-tag-scope.scope-public {
  background: rgba(59, 130, 246, 0.12);
  color: #2563eb;
  border-color: rgba(59, 130, 246, 0.2);
}

.kb-tag-scope.scope-department {
  background: rgba(16, 185, 129, 0.12);
  color: #059669;
  border-color: rgba(16, 185, 129, 0.18);
}

.kb-tag-scope.scope-private {
